...
class BookShelfTest extends FeatureSpec with GivenWhenThen with MustMatchers with TestFixtures with LogHelper {
  ...
  scenario("The list of books to read is printed as JSON") {
    ...
    val booksAsJSON = bookShelf.printAsJson
    Then("a valid JSON document is returned")
    booksAsJSON.indexOf( """{"books" : [{""") must be === 0
    3 must be === bookShelf.getAllBooks.size
    val booksFromJSONSet = bookShelf.getBooksFromJSON(booksAsJSON).toSet
    val diff = booksFromJSONSet.--(bookShelf.getAllBooks)
    0 must be === diff.size
  }
  ...
  scenario("Comparing the list of books stored on disk and the list retrieved from the Internet, create a list of books to read") {
    ...
    val booksToBeRead: List[Book] = bookShelf.getNewBooks(booksFromFile, booksFromWeb)
